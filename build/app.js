  ...
  createClass(TimestampHeader, [{
    ...
    value: function widgetRender(pathname) {
      ...
      if (this.props.fetchCallback) {
        ...
        var url = 'http://web.archive.org/cdx/search?url=' + domain + '/&status=200&fl=timestamp,digest&output=json';
        fetch(url).then(function (response) {
          return response.json();
        }).then(function (data) {
          _this2.prepareData(data);
...
var DiffContainer = function (_React$Component) {
  ...
  createClass(DiffContainer, [{
    ...
    value: function exportParams(path) {
      ...
      if (this.state.selectedMethod !== undefined) {
        ...
        if (/[0-9]{14}\/[0-9]{14}\/.+/.test(path)) {
          ...
          var timestampA = path[path.length - 3];
          var timestampB = path[path.length - 2];
          var urlA = 'http://web.archive.org/web/' + timestampA + '/' + site;
          var urlB = 'http://web.archive.org/web/' + timestampB + '/' + site;
    ...
    key: 'checkURL',
    value: function checkURL(urlA, timestampA, urlB, timestampB) {
